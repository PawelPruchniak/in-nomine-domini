...
import AddLettersForm from '../../../components/CMS/AddLettersForm/AddLettersForm';
        ...
        if (!localStorage.getItem('token')) {
            ...
            if (response.ok) {
                const res = await fetch(
                    `${backendDomain}/api/exerciseLetters/${params.id}`
                ).then((response) =>
    ...
    const update = async () => {
        ...
        if (res) {
            const response = await fetch(
                `${backendDomain}/security/exerciseLetters/${params.id}`,
                {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
